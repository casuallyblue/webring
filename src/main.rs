...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let app = Router::new()
        ...
        .route(
            "/",
            get(|State(state): State<Arc<AppState>>| async { HomePage { state }.render() }),
        )
        .route(
            "/join",
            get(|State(state): State<Arc<AppState>>| async { join::JoinPage { state }.render() }),
        )
        .route(
            "/members",
            get(|State(state): State<Arc<AppState>>| async {
                members::MembersPage { state }.render()
            }),
        )
        ...
        .route(
            "/feeds.opml",
            get(|State(state): State<Arc<AppState>>| async { feeds::feeds_opml(state) }),
        )
